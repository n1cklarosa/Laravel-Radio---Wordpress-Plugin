...
class Acw_Radio_Media {
	...
	private function sideload() {
		...
		if ( is_wp_error( $temp_file ) ) {
            error_log("error downloading file $this->url");
			return false;
		}
		...
		if ( ! $this->is_supported_image_type( $mime_type ) ) {
            error_log("fuiletype not supported $mime_type $this->url");
			return false;
		}

		// An array similar to that of a PHP `$_FILES` POST array
		$file = array(
			'name'     => explode("?", $this->get_filename( $mime_type ))[0],
			'type'     => $mime_type,
			'tmp_name' => $temp_file,
			'error'    => 0,
		...
		if ( $this->did_a_sideloading_error_occur( $file_attributes ) ) {
            error_log("sideload error occured ". $file_attributes['error'] . " " .$this->get_filename( $mime_type ) . " ". $mime_type);
			return false;
		}
